...
This document defines **mandatory architectural, layering, and coding rules**
for LazyPizza.

When performing a **code review**, the reviewer (human or AI) must:

- Treat these rules as **hard constraints**
- Flag any violation explicitly
- Prefer rejecting a change over allowing rule drift
- Assume the author is an experienced Android developer
...
- **Blocker**:
    - Any violation of layer boundaries
    - Forbidden dependencies
    - Business logic in UI
    - Domain leaking framework types

- **Strong Recommendation**:
    - Naming inconsistencies
    - Overly generic abstractions
    - State modeling that violates UDF principles

- **Suggestion**:
    - Readability improvements
        - Minor Compose performance optimizations

- **Review Code Quality**:
- Do not assume missing context is an error.
- Review only what is present in the diff.
- Do not invent architectural requirements not listed here.

---

## Best Practices Review (Supplementary)

In addition to enforcing the rules defined in this document, the reviewer must:

- Review the code against **latest Android, Kotlin, and Jetpack best practices**
- Consider recent guidance from:
    - Jetpack Compose
    - Coroutines & Flow
    - Hilt
    - Room
...
- Best practices must **not override** any rule in this document
- If a best practice conflicts with LazyPizza rules, LazyPizza rules win
- Any suggested improvement must:
    - Be clearly justified
    - Be actionable
    - Avoid speculative or trend-driven advice

If a best practice is missing from this document and would materially improve:
- correctness
- performance
- testability
- maintainability

the reviewer should explicitly point it out as a **suggested guideline addition**.
